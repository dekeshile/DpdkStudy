...
make_dpdk()
{
    ...
    for net_name in ${need_binds[@]};do
        $RTE_SDK/usertools/dpdk-devbind.py -s | grep $net_name
        if [ $? -eq 0 ]
        then
            ifconfig $net_name down  #先关闭网卡
            $RTE_SDK/usertools/dpdk-devbind.py --bind=igb_uio $net_name #绑定UIO模块到网卡
        else
            echo "*************can not find Interface:$net_name,Please check it***************8"
            #因脚本中执行异常，恢复为原来的环境
            restore_env
            exit
        fi
    ...
    #验证网卡绑定igb_uio的情况
    for net_name in ${need_binds[@]};do
        $RTE_SDK/usertools/dpdk-devbind.py -s | grep "drv=igb_uio"
        if [ $? -eq 0 ] ; then
...
then
   kill -9 $dpdk_pid
   echo "dpdk process is running [$dpdk_pid]，shutdown it "
fi
...
if [ -n "$suricata_pid" ]; 
then
   echo "suricata process is running [$suricata_pid] ,shutdown it"
