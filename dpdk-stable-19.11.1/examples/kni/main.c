 ...
 */
static void
kni_ingress(struct kni_port_params *p,uint16_t queueid )
{
	uint16_t port_id;
	unsigned nb_rx, num;
	...
	*/
	...
	//处理设置MAC地址这些请求
	rte_kni_handle_request(p->kni[queueid]);
...
init_port(uint16_t port)
{
	...
	//建立接收队列，一个dpdk端口对应可能对应多个接收队列，多个kni设备
	uint16_t queueid =0;
	int i = port;
	uint32_t j;
	for (j = 0; j < p[i]->nb_lcore_k; j++)//端口port_id有几个kni核,就对应有几个接收队列
	{
		queueid = j;
		ret = rte_eth_rx_queue_setup(port, queueid, nb_rxd,
			rte_eth_dev_socket_id(port), &rxq_conf, pktmbuf_pool);

		if (ret < 0){
			printf("Could not setup up RX queue for "
				"port%u (%d)\n", (unsigned)port, ret);
...
kni_alloc(uint16_t port_id)
{
	...
	for (i = 0; i < params[port_id]->nb_kni; i++) {
		...
		if (i == 0) {
			...
			//回调使用
			ops.config_network_if = kni_config_network_interface;//第一张KNI网卡可以配置为以太网虚拟网卡
			ops.config_mac_address = kni_config_mac_address;//第一张KNI网卡可以配置MAC地址
