...
main(int argc, char **argv)
{
	...
	/* parse application arguments (after the EAL ones) 包括初始化config参数 */
	...
	*/
	RTE_ETH_FOREACH_DEV(portid) {
		...
		nb_rx_queue = get_port_n_rx_queues(portid);//端口portid使用的接收队列数
		n_tx_queue = nb_lcores;//发送队列的使用核数
	...
	printf("\n");
	//如果命令行参数有有-parse-ptype选项，则启用软件去分析包的类型，如果没有这个选项，硬件会检查包类型
	for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
		if (rte_lcore_is_enabled(lcore_id) == 0)
			continue;
		qconf = &lcore_conf[lcore_id];
