...
printf("nb_lcore_params[%d]\n",nb_lcore_params);
	for (i = 0; i < nb_lcore_params; ++i) {
		printf("lcore_params[%d].port_id[%d],queue_id[%d],lcore_id[%d]\n",i,lcore_params[i].port_id,lcore_params[i].queue_id,lcore_params[i].lcore_id);
...
_parse_args()
{
	...
	int ret;
	/* call --config() on every slave lcore */
	int lcore_nums = 0;
	unsigned lcore_id;
	unsigned cur_lcore_id = 0;
	unsigned first_lcore_id;
	first_lcore_id = rte_lcore_id();
	RTE_LCORE_FOREACH_SLAVE(lcore_id) 
	{
		printf("!!!!!!!!!!!lcore_id[%d]\n",lcore_id);
		
		
		if(first_lcore_id > lcore_id)
		{
			first_lcore_id = lcore_id;
		}
		lcore_nums++;//rte_eal_remote_launch(lcore_hello, NULL, lcore_id);
	}
	printf("************lcore_nums[%d]\n",lcore_nums);
	...
	if(lcore_nums == 0 || port_nums == 0)
	{
		fprintf(stderr, "Error during getting lcore_id ( %d) portid ( %d)\n",
				lcore_nums,port_nums);
			return -1;
	}
	
	//cur_lcore_id = 0;
	nb_lcore_params = 0;
	cur_lcore_id = first_lcore_id;
	...
	RTE_ETH_FOREACH_DEV(portid) 
	{
		...
		for(i = 0;i < dev_info.max_rx_queues;++i)
		{
			...
			lcore_params_array[nb_lcore_params].queue_id =
				(uint8_t)i;
			lcore_params_array[nb_lcore_params].lcore_id =
				(uint8_t)cur_lcore_id++;
			if(cur_lcore_id > lcore_nums)
			{
				cur_lcore_id = first_lcore_id;//0;//first_lcore_id;
			}
			++nb_lcore_params; 
		}
	}
...
main(int argc, char **argv)
{
	...
	*/
	...
	/*
		...
		n_tx_queue = 0;//nb_rx_queue;//nb_lcores;//0;//nb_rx_queue-1;;//0;//nb_rx_queue;//nb_lcores;
		printf("nb_rx_queue[%d],n_tx_queue[%d]\n",nb_rx_queue,n_tx_queue);
		///
