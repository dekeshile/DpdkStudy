...
static struct kni_port_params kni_port_queue_socket_array[8];
...
#define ETH_RSS_E1000_IGB (\
    ...
    ETH_RSS_IPV6_TCP_EX | \
    ETH_RSS_IPV6_UDP_EX)
static uint8_t rss_intel_key1[40] = { 0x6D, 0x5A, 0x6D, 0x5A, 0x6D, 0x5A, 0x6D, 0x5A,
...
static struct kni_interface_stats kni_stats[RTE_MAX_ETHPORTS][MAX_RX_QUEUE_PER_LCORE];
...
static int32_t f_stop;
static int32_t f_pause;
unsigned int num_of_kni_ports;
...
{
	uint16_t i,j;
	struct rte_eth_stats eth_status;
	

	printf("\n**dpdk application statistics**\n"
	       "======  ==============  ============  ============  ============  ============\n"
	       " Port/Queue    Lcore(RX/TX)    rx_packets    rx_dropped    err_packets    device_dropped\n"
	       "------  --------------  ------------  ------------  ------------  ------------\n");
	for (i = 0; i < RTE_MAX_ETHPORTS; i++) 
	{
		...
		for(j = 0; j < kni_port_params_array[i]->queue_num;j++)
		{
			if(j == 0)
			{
				rte_eth_stats_get(i, &eth_status);	
			}
			else
			{
					eth_status.rx_nombuf = 0;
					eth_status.imissed = 0;
			}
			/*
			if(kni_stats[i][j].queue_on != 1)
				break;*/
			printf("%7d/%d %10u/%2u %13"PRIu64" %13"PRIu64" %13"PRIu64" "
							"%13"PRIu64"\n", i,j,
					kni_port_params_array[i]->lcore_rx,
					kni_port_params_array[i]->lcore_tx,
						kni_stats[i][j].rx_packets,
						kni_stats[i][j].rx_dropped,
						eth_status.rx_nombuf,
						eth_status.imissed);
		}
	}
	printf("======  ==============  ============  ============  ============  ============\n");
...
signal_handler(int signum)
{
	...
	if (signum == SIGRTMIN || signum == SIGINT){
		...
		printf("\nkni SIGRTMIN/SIGINT received. KNI processing stopping.\n");
		rte_atomic32_inc(&kni_stop);
		print_stats();
		force_quit = true;
		printf("kni force_quit[%d]\n",force_quit);
		return;
        }*/
}

void printf_kni_status()
{
		print_stats();
}
 ...
 */
 /*
static void
kni_ingress_bak(struct kni_port_params *p)
{
	uint8_t i;
	uint16_t port_id;
	unsigned nb_rx, num;
...
kni_ingress(struct kni_port_params *p)
{
	...
	for (i = 0; i < nb_rx; i++) {
		...
		// Burst tx to kni 
		rss_flag = pkts_burst[i]->hash.rss%nb_kni;
		num = rte_kni_tx_burst(p->kni[rss_flag], &pkts_burst[i], 1);
		if (num)
			kni_stats[port_id].rx_packets += num;

		//rte_kni_handle_request(p->kni[rss_flag]);
		if (unlikely(num < 1)) {
			 //Free mbufs not tx to kni interface 
...
int kni_status_change(uint16_t port_id)
{
	...
	if (f_pause)
		return -3;*/
	uint32_t nb_kni;
	nb_kni = kni_port_params_array[port_id]->nb_kni;
	uint8_t i;
	for (i = 0; i < nb_kni; i++)
	{
		rte_kni_handle_request(kni_port_params_array[port_id]->kni[i]);
	}
	return 0;
}
int pause_kni()
{
	return rte_atomic32_read(&kni_pause);
}
void kni_status_init(uint16_t port_id,uint8_t queueid)
{
	kni_stats[port_id][queueid].queue_on = 1;
}
void kni_ingress_nonuma(uint16_t port_id,uint8_t queueid)
{
	uint8_t i;
	//uint16_t port_id;
	unsigned nb_rx, num;
	...
	nb_kni = kni_port_params_array[port_id]->nb_kni;
	port_id = kni_port_params_array[port_id]->port_id;
	/* Burst rx from eth */
	nb_rx = rte_eth_rx_burst(port_id, queueid, pkts_burst, PKT_BURST_SZ);
	if (unlikely(nb_rx > PKT_BURST_SZ)) {
		RTE_LOG(ERR, APP, "Error receiving from eth\n");
		return;
	}
	...
	*/
	...
	for (i = 0; i < nb_rx; i++) 
	{
		...
		/* Burst tx to kni */
		rss_flag = pkts_burst[i]->hash.rss%nb_kni;
		num = rte_kni_tx_burst(kni_port_params_array[port_id]->kni[rss_flag], &pkts_burst[i], 1);
		if (num)
			kni_stats[port_id][queueid].rx_packets += num;

		//rte_kni_handle_request(p->kni[rss_flag]);
		if (unlikely(num < 1)) {
			/* Free mbufs not tx to kni interface */
			kni_burst_free_mbufs(&pkts_burst[num], 1);
			kni_stats[port_id][queueid].rx_dropped += 1;
		}
	}
}
void kni_ingress_numa(uint16_t port_id,uint8_t queueid,int socketid)
{
	uint8_t i;
	//uint16_t port_id;
	unsigned nb_rx, num;
	...
	nb_kni = kni_port_queue_socket_array[socketid].nb_kni;
	port_id = kni_port_params_array[port_id]->port_id;
	/* Burst rx from eth */
	nb_rx = rte_eth_rx_burst(port_id, queueid, pkts_burst, PKT_BURST_SZ);
	if (unlikely(nb_rx > PKT_BURST_SZ)) {
	...
	for (i = 0; i < nb_rx; i++) 
	{
		...
		//printf("rss[%u]\n",pkts_burst[i]->hash.rss);
		num = rte_kni_tx_burst(kni_port_queue_socket_array[socketid].kni[rss_flag], &pkts_burst[i], 1);
		if (num)
			kni_stats[port_id][queueid].rx_packets += num;
...
bak_config_kni(void)
{
	...
	for (i = 0; i < RTE_MAX_ETHPORTS; i++) 
	{
		if (!p[i])
			continue;
		printf("p[%d]->nb_lcore_k[%d]\n",i,p[i]->nb_lcore_k);
			
		if(p[i]->nb_lcore_k >= PORT_KNI_NUMS)
			continue;
			
		for(k = p[i]->nb_lcore_k; k < PORT_KNI_NUMS; k++)
		{
			for (j = 0; j < p[i]->nb_lcore_k; j++)
			{
				p[i]->lcore_k[k] = p[i]->lcore_k[j];
				k++;
				j++;
			}
			j = 0;
...
_config_kni(void)
{
	...
	int src_num;
	struct kni_port_params **p = kni_port_params_array;
	num_of_kni_ports *= PORT_KNI_NUMS;

	for (i = 0; i < nb_lcore_params; ++i) 
	{
		printf("lcore_params[%d].port_id[%d],queue_id[%d],lcore_id[%d]\n",
		i,
		lcore_params[i].port_id,
		lcore_params[i].queue_id,
		lcore_params[i].lcore_id);
		num_of_kni_ports++;
		p[lcore_params[i].port_id]->lcore_k[p[lcore_params[i].port_id]->nb_lcore_k] = lcore_params[i].lcore_id;
		p[lcore_params[i].port_id]->nb_lcore_k++;
	...
	for (i = 0; i < RTE_MAX_ETHPORTS; i++) 
	{
		if (!p[i])
			continue;
		src_num = p[i]->nb_lcore_k;
		p[i]->nb_lcore_k *= PORT_KNI_NUMS;
		printf("p[%d]->nb_lcore_k[%d]\n",i,p[i]->nb_lcore_k);
		j = 0;
		for(k = src_num;k < p[i]->nb_lcore_k;++k)
		{
			if(j >= src_num)
			{
				j = 0;	
...
_parse_args()
{
	...
	for (i = 0; i < nb_lcore_params; ++i)
	{
		...
		if (kni_port_params_array[port_id]) 
		{/*
			if(kni_port_params_array[port_id]->nb_lcore_k >= KNI_MAX_KTHREAD)
			{
				printf("Port %d has error kni\n", port_id);
				goto fail;
			}
			kni_port_params_array[port_id]->nb_lcore_k++;
			kni_port_params_array[port_id]->lcore_k[j] =
						lcore_params[i].lcore_id;*/
			continue;
			//printf("Port %d has been configured\n", port_id);
		...
		kni_port_params_array[port_id]->queue_num = dev_info.max_rx_queues;
		/*kni_port_params_array[port_id]->lcore_rx =
					(uint8_t)lcore_params[i].lcore_id;
		kni_port_params_array[port_id]->lcore_tx =
					(uint8_t)lcore_params[i].lcore_id;
		if (kni_port_params_array[port_id]->lcore_rx >= RTE_MAX_LCORE ||
		kni_port_params_array[port_id]->lcore_tx >= RTE_MAX_LCORE) {
...
init_port(uint16_t port)
{
	...
	ret = rte_eth_rx_queue_setup(port, 0, nb_rxd,
		rte_eth_dev_socket_id(port), &rxq_conf, pktmbuf_pool[port][rte_eth_dev_socket_id(port)]);
	if (ret < 0)
...
kni_change_mtu(uint16_t port_id, unsigned int new_mtu)
{
	...
	ret = rte_eth_rx_queue_setup(port_id, 0, nb_rxd,
		rte_eth_dev_socket_id(port_id), &rxq_conf, pktmbuf_pool[port_id][rte_eth_dev_socket_id(port_id)]);
	if (ret < 0) {
...
kni_alloc(uint16_t port_id)
{
	...
	for (i = 0; i < params[port_id]->nb_kni; i++) 
	{
		...
		if (i == 0) 
		{
			...
			lcore_id = params[port_id]->lcore_k[i];
			...
			if (!per_port_pool)
			{
				kni = rte_kni_alloc(pktmbuf_pool[0][socketid], &conf, &ops);	
				kni_port_queue_socket_array[socketid].kni[kni_port_queue_socket_array[socketid].nb_kni] = kni;
				kni_port_queue_socket_array[socketid].nb_kni++;
			}
			else
			{
				kni = rte_kni_alloc(pktmbuf_pool[port_id][socketid], &conf, &ops);	
		...
		} else
		{
			...
			if (!per_port_pool)
			{
				kni = rte_kni_alloc(pktmbuf_pool[0][socketid], &conf,NULL);	
				kni_port_queue_socket_array[socketid].kni[kni_port_queue_socket_array[socketid].nb_kni] = kni;
				kni_port_queue_socket_array[socketid].nb_kni++;
			}
			else
			{
				kni = rte_kni_alloc(pktmbuf_pool[port_id][socketid], &conf, NULL);	
...
static int
kni_alloc_all(uint16_t port_id)
{
	uint8_t i;
	struct rte_kni *kni;
	struct rte_kni_conf conf;
	...
	for (i = 0; i < params[port_id]->nb_kni; i++) 
	{
		...
		lcore_id = params[port_id]->lcore_rx;
		/*
		 * The first KNI device associated to a port
		 * is the master, for multiple kernel thread
		 * environment.
...
/* Initialise ports/queues etc. and start main loop on each core */
	...
	static uint16_t nb_sys_ports, port;
...
int kni()
{
	...
	memset(&kni_port_queue_socket_array,0,sizeof(kni_port_queue_socket_array));
...
/* free ports/queues etc. and start main loop on each core */
int free_kni()
{
	int i;
		monitor_links = 0;
